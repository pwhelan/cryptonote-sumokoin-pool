...
function payoutPayments(payments, callback) {
    ...
    var payment_id = randomValueHex(64);
    redisClient.incr(config.coin + ":payout_id", function(err, payout_id) {
        redisClient.hset(config.coin + ":payouts", payout_id, payment_id);
        redisClient.hmset(config.coin + ":payout:" + payout_id, 
    ...
    for (var worker in payments) {
        ...
        tx.redis.push(['hmset', config.coin + ":payout:" + payment_id, worker, amount]);
        tx.amount += amount;
