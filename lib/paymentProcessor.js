...
function filterBalances(balances, minPayoutLevel, callback) {
    ...
    for (var worker in balances) {
        ...
        if (balance - minerTransferFee() >= minPayoutLevel[worker]) {
            var remainder = balance % config.payments.denomination;
            var payout = balance - remainder;
            // check to see if they get paid.
            if ((payout + minerTransferFee()) <= 0) continue;
...
function payoutPayments(payments, callback) {
    ...
    for (var worker in payments) {
        ...
        tx.dest.push({amount: amount - minerTransferFee(), address: worker});
        tx.redis.push(
            ['hincrby', config.coin + ':workers:' + worker, 'balance', -amount],
            ['hincrby', config.coin + ':workers:' + worker, 'paid', amount - minerTransferFee()]
        );
        tx.amount += amount;
    }
